    ...
    return new Promise((resolve, reject) => {
        ...
        request.query("SELECT * FROM " + tableName, async (err, rows) => {
            ...
            collection.insertMany(record_rows, (err, res) => {
                if(err) {
                    logger.error(`  ERROR Inserting table ${tableName} : ${err}`);
                    resolve(true);
                    return;
                }
                if(res.insertedCount !== nrows) {
                    logger.error(`  ERROR Inserting table ${tableName} : Inserted ${res.insertedCount} of ${nrows}`);
                    resolve(true);
                    return;
                }
                ...
                logger.info(`  Inserting table ${tableName} finished in ${time_insert_s} rows: ${res.insertedCount} (rows/s): ${(res.insertedCount/time_insert_s).toFixed()}`);
                resolve(true);
                return;
            });
        })
